...
contract KIP17Enumerable is KIP13, KIP17, IKIP17Enumerable {
    ...
    mapping(uint256 => uint256) private _allTokensExpire;
    
    uint256 private _stamp;
     ...
     */
    constructor (uint256 expireDay, uint256 stampCount) public {
    ...
    function expire() public view returns (uint256) {
        return _expire / 1 days;
    }
    
    function setExpire(uint256 expireDay) public {
        require(msg.sender == _manager, 'only manager');
        _expire = expireDay * 1 days;
    }
    
    function tokenExpire(uint256 tokenId) public view returns (uint256) {
        return _allTokensExpire[tokenId];
    }
    
    function stamp() public view returns (uint256) {
        return _stamp;
    }
    
    function setStampCount(uint256 stampCount) public {
        require(msg.sender == _manager, 'only manager');
        _stamp = stampCount;
    }
    ...
    function mint(address to) public {
        require(msg.sender == _manager, 'only manager');
        uint256 tokenId = _allSupply;
        _allTokensExpire[tokenId] = now + _expire;
    ...
    function burn(address owner) public {
        require(msg.sender == _manager, 'only manager');
        require(balanceOf(owner) >= _stamp, 'stamp count');
        
        uint256[] memory burnToken = _ownedTokens[owner];
        uint8 check;
        
        for(uint8 i=0; i < burnToken.length; i++) {
            uint256 tokenId = burnToken[i];
            
            if(_allTokensExpire[tokenId] >= now) {
                check++;
            }
